...
项目位于 `D:\MyProgram\astral-atlas`。原空目录 `astronony` 被工作区进程占用，无法在本次会话中直接重命名，因此建立了目标目录，未覆盖或删除原目录。关闭旧工作区后可自行移除空目录，并打开新项目目录。

## 验证

`npm test` 检查 NOAA 解析、UTC、去重、异常输入、时效边界、AU 换算与文章内容结构。`npm run build` 包含严格 TypeScript 检查。浏览器检查应覆盖桌面与手机、键盘导航、数据失败/恢复、WebGL 降级与减少动态效果。
